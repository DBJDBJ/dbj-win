...
extern "C" {
	...
	static inline dbj_a_string dbj_a_string_make(const char * str_a_)
	{
		...
		HRESULT hr_ = dbjwin_sprintfa(dbj_a_string.data, dbj_a_w_string_max_size, "%s", str_a_);
	...
	static inline dbj_w_string dbj_w_string_make( const wchar_t * str_w_ )
	{
		...
		HRESULT hr_ = dbjwin_sprintfw(wstring_.data, dbj_a_w_string_max_size, L"%s", str_w_ );
		DBJ_VERIFYX( hr_ == S_OK );
		return wstring_;
	}
